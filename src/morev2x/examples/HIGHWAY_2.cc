...
//      m_size = AperiodicPKTs_Size[packetSize_index->GetInteger(0,AperiodicPKTs_Size.size()-1)];
   ...
   //   ReservationSize = LargestPeriodicSize;
    ...
    //std::cout << "SUMO DEBUG - position [ " << xPosition << ", " << yPosition << " ]" << std::endl;
...
//      std::cout << ueNode -> GetObject<Ipv4> () -> GetAddress(1,0).GetLocal() << std::endl;
  ...
  //-------------------------------------------------------------------------------//
  // Moved application settings down here
...
//  groupAddress = Ipv4AddressGenerator::NextAddress (Ipv4Mask ("255.0.0.0"));
  ...
  proseHelper->ActivateSidelinkBearer (Seconds(0.0), ueDevs, tftRx); // Set accordingly with clientApps StartTime! 0.1 Seconds is the ActivationTime of the Sidelink bearer
...
//  std::cin.get();
  ...
  STARTUP_FCN setupNewWifiNode = [&] (std::string vehicleID) -> Ptr<Node>
    {
      if (nodeCounter >= ueResponders.GetN())
        NS_FATAL_ERROR("Node Pool empty!: " << nodeCounter << " nodes created.");

      Ptr<Node> includedNode = ueResponders.Get(nodeCounter);
      ++nodeCounter; // increment counter for next node

      /* Install Application */
      ApplicationContainer clientApp = udpClient.Install(includedNode);
      ApplicationContainer clientRespondersSrvApp = clientPacketSinkHelper.Install(includedNode);

//      clientRespondersSrvApps.Get(nodeCounter) -> SetStartTime(Seconds (0.0));
//      clientRespondersSrvApps.Get(nodeCounter) -> SetStopTime (simulationTime - Simulator::Now () - Seconds (0.1)); // The simulation ends at simTime + 1, so we have a buffer of 0.5s

//      clientApps.Get(nodeCounter) -> SetStartTime(Seconds (0.0));
//      clientApps.Get(nodeCounter) -> SetStopTime (simulationTime - Simulator::Now () - Seconds (0.1)); // The simulation ends at simTime + 1, so we have a buffer of 0.5s

      clientApp.Start (Seconds (0.0));
      clientApp.Stop (simulationTime - Simulator::Now () - Seconds (0.1));
      clientRespondersSrvApp.Start (Seconds (0.0));
      clientRespondersSrvApp.Stop (simulationTime - Simulator::Now () - Seconds (0.1));
